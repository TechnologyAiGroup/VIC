...
def train(trainset, testset):
    ...
    _test_untagged_words = [[tup[0] for tup in sent] for sent in test_set]
    ...
    original_order = []
    difference = 0
    for _test_untagged_words_perchip in _test_untagged_words:
        start = time.time()
        tagged_seq = Viterbi(_test_untagged_words_perchip, train_bag=train_tagged_words, tags_df=tags_df)
        difference += time.time()-start
        pred_set.append(tagged_seq)
        tmp = []
        for tup in tagged_seq[:-1]:
            tmp.append(tup[0])
...
def check(sort_optimized, ground_true, quantile):
    ...
    count = 0
    for i in range(len(sort_optimized)):
        length = int(quantile * len(sort_optimized[i]))
        if length == 0:
            length = 1
        ...
        for candidate in retain_candidates:
            if candidate in fault_set:
                count += 1
                break
    acc_for_chip = count / count_complete
    return acc_for_chip
...
if __name__ == '__main__':
    ...
    for i in range(len(train_clusters_data)):
        ...
        if testset == [0]:
            continue
        # 按candidate长度升序排序
        # trainset = sorted(trainset, key=len)
        # testset = sorted(testset, key=len)
    ...
    res_folder = "experiment/res_perchip"
